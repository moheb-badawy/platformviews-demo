...
class FLNativeView: NSObject, FlutterPlatformView {
    ...
    func createNativeView(view _view: UIView, args: Any?){
        ...
        let fileName = argumentsDictionary?["filePath"] as? String
        _view.backgroundColor = UIColor.blue
        let nativeLabel = UILabel()
        if let fileName = argumentsDictionary?["filePath"] as? String {
            nativeLabel.text = "Native text from iOS \(fileName)"
        } else {
